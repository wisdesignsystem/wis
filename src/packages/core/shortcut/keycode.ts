 ...
 */
export function keyCodeMapper(keyCode: number) {
	if (
		[KEY_CODE_LEFT_CMD, KEY_CODE_RIGHT_CMD, KEY_CODE_FIREFOX_META].includes(
			keyCode,
		)
	) {
	...
	const numpadMap: { [key: number]: number } = {
...
export function isCommandKey(keyCode: number) {
	return [
		KEY_CODE_CONTROL,
		KEY_CODE_SHIFT,
		KEY_CODE_ALT,
		KEY_CODE_META,
	].includes(keyCodeMapper(keyCode));
}

const localKeyMap: { [key: string]: number } = {
...
export const keyMap = Object.keys(localKeyMap).reduce(
	(result, key) => {
